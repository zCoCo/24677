...
class CustomController(BaseController):
    ...
    def __init__(self, trajectory):
        ...
        self.look_ahead_multiple = 5.5 # How many car lengths to look ahead and average over when determining desired heading angle
    ...
    def update(self, timestep):
        ...
        # Pole Placement:
        fsf = signal.place_poles(A,B, self.desired_poles)
        K = fsf.gain_matrix.reshape(1,-1)
        ...
        e2 = np.arctan2(np.sin(psi), np.cos(psi)) - desired_heading
        ...
        Q = np.asarray([e1,e1dot,e2,e2dot]).reshape(-1,1) # States
